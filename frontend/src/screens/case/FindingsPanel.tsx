  ...
  if (findings.length === 0) {
    return (
      <EmptyState
        icon={CheckCircle2}
        title="No findings"
        description="Every rule passed on this case — nothing needed a human decision."
      />
    );
  }
      ...
      {sorted.map((finding) => (
        <li key={finding.id} className="px-4 py-3.5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge tone={STATUS_TONE[finding.status]}>{finding.status}</Badge>
          ...
          <code className="mt-1 block text-caption text-ink-2/80">{finding.code}</code>
